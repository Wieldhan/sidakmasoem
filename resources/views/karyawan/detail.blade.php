			...
			<div class="card card-primary card-outline">
				<div class="card-body box-profile">
					<div class="text-center">
						...
						@else
						<img style="width:150px; height: 150px;" class="img-thumbnail rounded-circle elevation-1"
							src="{{asset('images/avatars/'.user()->avatar)}}" alt="profile picture">
						@endif
					</div>
					<h3 class="profile-username text-center" style="font-size: 20px;">{{user()->karyawan->nama_lengkap}}
					</h3>
					<p class="text-muted text-center">{{user()->karyawan->jabatan->jabatan}}</p>
					<ul class="list-group list-group-unbordered mb-3">
						<li class="list-group-item">
							<a>Jenis Kelamin</a>
							<span class="float-right">{{user()->karyawan->jk}}</span>
						</li>
						<li class="list-group-item">
							<a>Agama</a>
							<span class="float-right">{{user()->karyawan->agama}}</span>
						</li>
						<li class="list-group-item">
							<a>Alamat</a>
							<span class="float-right" style="text-align: justify;">{{user()->karyawan->alamat}}</span>
						</li>
						<li class="list-group-item">
							<a>No Telepon</a>
							<span class="float-right">{{user()->karyawan->no_telepon}}</span>
						</li>
						<li class="list-group-item">
							<a>Email</a>
							<span class="float-right">{{user()->karyawan->user->email}}</span>
						</li>
						<li class="list-group-item">
							<a>Visi</a>
							<span class="float-right">{{user()->karyawan->visi}}</span>
						</li>
						<li class="list-group-item">
							<a>Misi</a>
							<span class="float-right">{{user()->karyawan->misi}}</span>
						</li>
					</ul>
