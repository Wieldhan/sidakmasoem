<!-- modal edit profil -->
<div class="modal fade modal-edit-profil" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel"
	aria-hidden="true">
				...
				<form action="/profile/update/{{auth::user()->id}}" method="POST">
					...
					<div class="form-row">
						<div class="form-group col-sm-6">
							...
							<input required name="nik" onkeypress="hanyaangka(event)" type="text" class="form-control form-control-sm"
								value="{{Auth::user()->karyawan->nik}}">
						</div>
						<div class="form-group col-sm-6">
							<label>No KTP</label>
							<input required name="no_ktp" onkeypress="hanyaangka(event)" type="text"
								class="form-control form-control-sm" value="{{Auth::user()->karyawan->no_ktp}}">
						</div>
					</div>
					<div class="form-row">
						<div class="form-group col-sm-6">
						...
						<div class="form-group col-sm-6">
							...
							<select required name="jk" class="form-control form-control-sm" id="formcontroljk">
								<option value="Laki Laki" {{Auth::user()->karyawan->jk === "Laki Laki"? "selected" : ""}}>Laki Laki
								</option>
								<option value="Perempuan" {{Auth::user()->karyawan->jk === "Perempuan"? "selected" : ""}}>Perempuan
								</option>
							</select>
						</div>
					</div>
					<div class="form-row">
						<div class="form-group col-sm-6">
							...
							<select required name="agama" class="form-control form-control-sm" id="formcontrolagama">
								...
								<option value="Protestan" {{Auth::user()->karyawan->agama === "Protestan"? "selected" : ""}}>Protestan
								</option>
								<option value="Khatolik" {{Auth::user()->karyawan->agama === "Khatolik"? "selected" : ""}}>Khatolik
								</option>
						...
						<div class="form-group col-sm-6">
							...
							<select required name="status_nikah" class="form-control form-control-sm" id="formcontrolpernikahan">
								<option value="Belum Menikah"
									{{Auth::user()->karyawan->status_nikah === "Belum Menikah"? "selected" : ""}}>Belum Menikah</option>
								<option value="Menikah" {{Auth::user()->karyawan->status_nikah === "Menikah"? "selected" : ""}}>Menikah
								</option>
								<option value="Cerai" {{Auth::user()->karyawan->status_nikah === "Cerai"? "selected" : ""}}>Cerai
								</option>
							</select>
						</div>
					</div>
					<div class="form-row">
						<div class="form-group col-sm-6">
							...
							<input required name="tempat_lahir" type="text" class="form-control form-control-sm"
								value="{{Auth::user()->karyawan->tempat_lahir}}">
						</div>
						<div class="form-group col-sm-6">
							<label>Tanggal Lahir</label>
							<input required name="tanggal_lahir" type="date" class="form-control form-control-sm"
								value="{{Auth::user()->karyawan->tanggal_lahir}}">
						</div>
					</div>
					<div class="form-row">
						<div class="form-group col-sm-6">
							<label>No Telepon</label>
							<input required name="no_telepon" type="text" class="form-control form-control-sm"
								value="{{Auth::user()->karyawan->no_telepon}}">
						</div>
						<div class="form-group col-sm-6">
							<label>Alamat</label>
							<textarea style="height: 75px;" required name="alamat"
								class="form-control">{{Auth::user()->karyawan->alamat}}</textarea>
						</div>
					</div>
					<div class="form-row">
						<div class="form-group col-sm-6">
							...
							<textarea style="height: 75px;" required name="visi"
								class="form-control">{{Auth::user()->karyawan->visi}}</textarea>
						</div>
						<div class="form-group col-sm-6">
							<label>Misi</label>
							<textarea style="height: 75px;" required name="misi"
								class="form-control">{{Auth::user()->karyawan->misi}}</textarea>
						</div>
					</div>
					<div class="row">
...
<!-- modal pendidikan -->
<div class="modal fade modal-pendidikan" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel"
	aria-hidden="true">
				...
				<form action="/profile/simpanpend" method="POST">
					...
					<div class="form-group col-sm-6">
						...
						<input required name="nik" type="text" class="form-control form-control-sm"
							value="{{Auth::user()->karyawan->nik}}" readonly="true">
					</div>
...
<div class="modal fade modal-pengalaman" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel"
	aria-hidden="true">
	<div class="modal modal-dialog modal-dialog-centered" role="document">
		<div class="modal-content">
			<div class="modal-header">
...
<div class="modal fade modal-organisasi" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel"
	aria-hidden="true">
	<div class="modal modal-dialog modal-dialog-centered" role="document">
		<div class="modal-content">
			<div class="modal-header">
